...
const MatchmakingPage: React.FC = () => {
  ...
  const generateMatchResults = async () => {
    ...
    try {
      ...
      // In a real application, this would be an API call to your backend with WatsonX SDK
      // For now, we'll simulate the response with a calculation similar to the WatsonX SDK matcher
      ...
      // Process each selected university
      for (let i = 0; i < selectedUniversities.length; i++) {
        const uniName = selectedUniversities[i];
        
        // Calculate match score factors
        const gpaFactor = preferences.gpa >= 3.5 ? 0.3 : 0.2;
        const languageFactor = preferences.languageScore >= 7.0 ? 0.2 : 0.1;
        const interestsFactor = preferences.interests.length >= 2 ? 0.2 : 0.1;
        
        // Ranking bonus - higher preference universities get a bonus
        // The bonus decreases as we go down the preference list
        const rankPosition = preferences.universities.findIndex(uni => uni === uniName);
        const rankingBonus = Math.max(0, 0.3 - (rankPosition * 0.03));
        
        // Base score between 6-8
        const baseScore = 6 + Math.random() * 2;
        
        // Calculate final score (out of 10)
        const finalScore = Math.min(10, baseScore + gpaFactor + languageFactor + interestsFactor + rankingBonus);
        const roundedScore = Math.round(finalScore * 10) / 10;
        
        // Generate match explanation
        const explanation = generateMatchExplanation(uniName, roundedScore, preferences, rankPosition);
  ...
  return (
    <div className="matchmaking-page">
      <div className="matchmaking-header">
        ...
        <p>Powered by IBM WatsonX AI, our matchmaking system analyzes your academic profile and preferences to find the perfect university matches for your exchange program.</p>
      </div>
        ...
        {activeTab === 'preferences' && (
          <div className="preferences-section">
            ...
            <div className="powered-by">
              <div className="watson-text">
                <p>Powered by IBM WatsonX</p>
                <p className="model-name">granite-13b-instruct-v2</p>
              </div>
            </div>
          </div>
        )}
